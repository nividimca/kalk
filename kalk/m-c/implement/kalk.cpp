...
kalk::kalk(){
    ...
    connect( viewK, SIGNAL( sendChangeLogic(int)), modelK, SLOT(changeLogic(int)));
    connect( viewK, SIGNAL( sendOpenInput()), this, SLOT(getOpenInput()));
    connect( viewK, SIGNAL( sendClearElement()), modelK, SLOT( clearElement()));
    ...
    connect( modelK, SIGNAL( sendChangedLogic()), this, SLOT( refreshKalk()));
}
